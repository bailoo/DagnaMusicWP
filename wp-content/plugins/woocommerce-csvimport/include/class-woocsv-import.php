...
class woocsv_import
{
    ...
    public function write_to_log ( $batch_code, $logs = array () ) {

        //open file
        $upload_dir = wp_upload_dir ();
        $filename = $upload_dir['basedir'] . '/csvimport/' . $batch_code . '.log';
        $fp = fopen ( $filename, 'a' );

        if ( is_string ( $logs ) ) {
            fwrite ( $fp, $logs . PHP_EOL );
            ...
            fwrite ( $fp, serialize($logs) . PHP_EOL );
        }
